...
export async function validateGateReady(gateId: string): Promise<void> {
  ...
  if (!content.includes('**Status**: completed')) {
    throw new ZenoError(
      `Gate ${gateId} is not completed`,
      'ARCHIVE_NOT_READY',
      { gateId, reason: 'Gate status is not completed' }
    )
  }
 ...
 */
export async function validateProposalReady(hash: string): Promise<{ type: 'gate-tied' | 'solitary'; gateId?: string; title: string }> {
  // Find proposal file
  ...
  if (existsSync(gatesDir)) {
    const gateDirs = (await import('node:fs/promises')).readdir(gatesDir)
  ...
  if (!content.includes('**Status**: completed')) {
    throw new ZenoError(
      `Proposal ${hash} is not completed`,
      'ARCHIVE_NOT_READY',
      { hash, reason: 'Proposal status is not completed' }
    )
  }
