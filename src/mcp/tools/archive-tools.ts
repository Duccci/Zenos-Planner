...
export function archiveHandlers(_registry?: FunctionRegistry) {
  const gateHandler = _registry ? createSchemaValidatingHandler(_registry, 'archive_gate', ArchiveGateOutputSchema) : undefined
  const proposalHandler = _registry ? createSchemaValidatingHandler(_registry, 'archive_proposal', ArchiveProposalOutputSchema) : undefined
  const batchHandler = _registry ? createSchemaValidatingHandler(_registry, 'archive_batch', ArchiveBatchOutputSchema) : undefined
  ...
  return {
    async archive_gate(args: Record<string, unknown>) {
      ...
      try {
        ...
        if (gateHandler) return gateHandler(args)
    ...
    async archive_proposal(args: Record<string, unknown>) {
      ...
      try {
        ...
        if (proposalHandler) return proposalHandler(args)
    ...
    async archive_batch(args: Record<string, unknown>) {
      ...
      try {
        ...
        if (batchHandler) return batchHandler(args)

        const validated = ArchiveBatchInputSchema.parse(args)
        const result = await archiveBatch(validated.artifacts as any, validated.completionNotes)
