...
export function findCodeWorkspaceFile(projectRoot: string): string | undefined {
  try {
    ...
    if (entries.length > 0) return join(projectRoot, entries[0]!)
  ...
  try {
    ...
    if (normalizePath(parent) !== normalizePath(projectRoot)) {
      ...
      if (parentEntries.length > 0) return join(parent, parentEntries[0]!)
    }
  } catch { /* directory unreadable */ }
...
export function extractWorkspaceFolders(
  ...
  try {
    ...
    const parsed = jsoncParse(content) as { folders?: Array<{ path: string; name?: string }> }
