...
export function registerTraceCommand(program: Command): void {
  program
    ...
    .action(async (artifactHash: string, options: {
      from?: string
      to?: string
      branch?: string
      limit?: string
      ...
      try {
        ...
        const result = await registry.invoke('git_trace', params)
        ...
        if (result.success) {
          if (options.json) {
            ...
            console.log(`Found ${data.commits.length} commits referencing ${artifactHash}`)
            console.log(`Total commits searched: ${data.totalCommits}`)
            console.log()
          ...
          logger.error(`Trace failed: ${result.error.message}`)
          process.exit(1)
        }
      } catch (error) {
