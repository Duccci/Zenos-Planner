...
**Acceptance**:
- [ ] Server restarts on source file changes
- [ ] Development mode can be enabled via environment variable or flag
- [ ] File watching properly cleaned up on shutdown
- [ ] No file watching in production (controlled by environment)
...
**Acceptance**:
- [ ] Health check reports all key status indicators
- [ ] Database connectivity tested
- [ ] Project configuration validated
- [ ] Output includes helpful troubleshooting hints
...
**Acceptance**:
- [ ] All tool invocations logged in debug mode
- [ ] Errors include full context (type, message, stack)
- [ ] Error history available for inspection
- [ ] Stack traces only shown in debug/development mode
...
**Acceptance**:
- [ ] Guide covers most common issues
- [ ] Debugging section helps diagnose root causes
- [ ] Examples provided for each troubleshooting step
- [ ] Links to logs and health check commands
...
- `zeno mcp logs [--count N] [--level debug|info|warn|error]` - Show server logs
...
**Acceptance**:
- [ ] All diagnostic commands implemented
- [ ] Output includes helpful hints and next steps
- [ ] Logs properly formatted for readability
- [ ] Tools list shows all ~20 tools with schemas
...
**Acceptance**:
- [ ] VS Code output panel shows MCP logs
- [ ] Health check accessible via Command Palette
- [ ] Error messages in Chat view are actionable
- [ ] Documentation covers diagnostic features
...
**Acceptance**:
- [ ] Health checks report all indicators correctly
- [ ] Error logging preserves context and stack traces
- [ ] Error history limited to reasonable size (last 50)
- [ ] Coverage ≥90% for diagnostics module
