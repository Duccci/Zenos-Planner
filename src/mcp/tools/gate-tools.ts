...
export const gateToolDefinitions = [
  {
    name: 'gates_action',
    description: `Gate lifecycle: list, show, generate, validate, start, complete, regenerate, cancel, defer. Gates use plaintext IDs in "gate-XX" format (e.g. "gate-08"); pass the gate ID directly as gateId. For generate: supply gateId+name+objectives to create a gate directly, or omit them to auto-generate from PRD — the AI decomposition path REQUIRES preReview with phase="generate". Read the full PRD and call reg_action { action: "list" } BEFORE calling generate, then supply preReview: { phase: "generate", openQuestionsResolved (bool), questionsFound (string[]), gateReviewed (bool), requirementsVerified (bool), vagueRequirements (string[]), assumptionsDocumented (string[]), blockersIdentified (string[]) }. Omitting preReview returns a structured validation error.`,
  ...
  const gateActionHandler = createEntityActionHandler(
    {
      ...
      actionHandlers: {
        ...
        validate: async (payload, r) => {
          ...
          // requirementsCoverage is informational only — not a blocking check.
          // Requirements are auto-generated by gates_action:start, but start requires
          // the gate to be 'validated' first. Blocking validate on requirements
          // creates an unsolvable circular dependency: validate→needs reqs→start→needs validated.
          // We emit a warning if no requirements exist so the LLM knows they will be
          // generated at start, but we never fail validation on this condition.
          ...
          // 5) Requirements coverage: informational warning only (not a blocker).
          // Requirements are generated at gates_action:start time; blocking validate on
          // their existence would make it impossible to ever validate a fresh gate.
          try {
            ...
            if (reqResult.success) {
              ...
              if (count === 0) {
                allWarnings.push(
                  `Gate ${gateId} has no requirements yet — they will be auto-generated when you run gates_action:start.`
                )
              }
            }
