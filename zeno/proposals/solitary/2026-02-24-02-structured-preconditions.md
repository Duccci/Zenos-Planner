...
**Acceptance Criteria**:
- [ ] G1–G9 entries in guardrails constants updated: `mustHaveValidator: true`, `validatorRef: 'proposal-tools.ts#preReview'` (or `gate-tools.ts#preReview`)
- [ ] G10–G11 entries updated: `mustHaveValidator: true`, `validatorRef: 'scope-validator.ts#validateTestFileScope'`
- [ ] G12 entry updated: `mustHaveValidator: true`, `validatorRef: 'scope-validator.ts#validateMarkdownOnly'`
- [ ] `tests/mcp/guardrail-coverage.test.ts` (from `s20260224skill01`) passes — all `mustHaveValidator: true` entries have valid validatorRefs
- [ ] No guardrail that has a structured precondition or deterministic validator remains in the narrative-only category
- [ ] Guardrail allowlist entries for G1–G12 patterns can be removed from `src/mcp/allowlists/guardrail-allowlist.ts` (they are no longer narrative-only)
