...
export async function decomposeToProposals(
  ...
  for (const objective of objectives) {
    ...
    let proposalContent = templateContent
      ...
      .replace(/\{\{REQUIREMENTS\}\}/g, requirements.map(r => `- #${r.id}: ${r.description}`).join('\n'))
    ...
    const filename = `${proposalIndex.toString().padStart(2, '0')}-${objective.toLowerCase().replace(/[^a-z0-9]+/g, '-').substring(0, 30)}.md`
...
export function calculateProposalDependencies(proposals: any[]): Array<{ from: string; to: string; type: string }> {
  const dependencies = []
