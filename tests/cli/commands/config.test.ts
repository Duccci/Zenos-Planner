...
describe('Config command action coverage', () => {
  ...
  describe('config show', () => {
    ...
    it('should get a specific key', async () => {
      ...
      // Use parent config --get (backward compat) since the show subcommand
      // shares the --get option name with its parent and Commander routes it there
      await program.parseAsync(['node', 'test', 'config', '--get', 'projectName'])

      expect(vi.mocked(logger.info)).toHaveBeenCalledWith('projectName: "TestProject"')
    })
    ...
    it('should get a nested key', async () => {
      ...
      await program.parseAsync(['node', 'test', 'config', '--get', 'git.autoCommit'])

      expect(vi.mocked(logger.info)).toHaveBeenCalledWith('git.autoCommit: true')
    })
    ...
    it('should exit when not in project', async () => {
      ...
      await expect(program.parseAsync(['node', 'test', 'config', 'show'])).rejects.toThrow()
      expect(exitSpy).toHaveBeenCalledWith(1)
    })
    ...
    it('should output JSON when ZENO_INTEGRATION is set', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(JSON.stringify({ projectName: 'Test' }, null, 2))
  ...
  describe('config set', () => {
    ...
    it('should parse JSON values', async () => {
      ...
      await program.parseAsync(['node', 'test', 'config', 'set', 'settings.count', '42'])
