...
3. **Using prompts**:
   ...
   - Type `/zeno-apply`, `/zeno-gate`, `/zeno-proposal`, `/zeno-archive`, or `/zeno-gate-complete`
...
### `/zeno-archive`
Archive a completed proposal and update dependent gates and requirements.

**Usage**: `/zeno-archive #p01projconf01`

### `/zeno-gate-complete`
Complete a gate by archiving it and consolidating associated proposals.

**Usage**: `/zeno-gate-complete gate-01`
...
- YAML frontmatter with metadata (`name`, `description`, `agent`, `model`)
...
These prompts are functionally identical to the Cursor commands in `.cursor/commands/`, but formatted for VS Code's prompt system:
- Uses `.prompt.md` extension instead of `.md`
- Located in `.github/prompts/` instead of `.cursor/commands/`
- YAML frontmatter includes `model: qwen3-coder` for Ollama compatibility
- Same content and guardrails as Cursor versions
