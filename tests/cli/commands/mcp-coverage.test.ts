...
describe('MCP commands action coverage', () => {
  ...
  describe('diagnostics subcommand', () => {
    ...
    it('should handle diagnostics errors', async () => {
      ...
      await expect(
        program.parseAsync(['node', 'test', 'mcp', 'diagnostics'])
      ).rejects.toThrow()
  ...
  describe('tools subcommand', () => {
    ...
    it('should handle tools listing errors', async () => {
      ...
      await expect(
        program.parseAsync(['node', 'test', 'mcp', 'tools'])
      ).rejects.toThrow()
      expect(exitSpy).toHaveBeenCalledWith(1)
    })
  })
  ...
  describe('errors subcommand', () => {
    ...
    it('should handle errors subcommand failure', async () => {
      ...
      await expect(
        program.parseAsync(['node', 'test', 'mcp', 'errors'])
      ).rejects.toThrow()
      expect(exitSpy).toHaveBeenCalledWith(1)
    })
  })
  ...
  describe('install subcommand', () => {
    it('should install MCP config with written=true', async () => {
      ...
      await expect(
        program.parseAsync(['node', 'test', 'mcp', 'install'])
      ).rejects.toThrow()
    ...
    it('should handle already existing config', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        '[mcp-install] Workspace MCP config already exists'
      )
    })
  ...
  describe('stop subcommand', () => {
    ...
    it('should handle stop errors', async () => {
      ...
      await expect(
        program.parseAsync(['node', 'test', 'mcp', 'stop'])
      ).rejects.toThrow()
      expect(exitSpy).toHaveBeenCalledWith(1)
    })
  })
