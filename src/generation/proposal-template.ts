...
export function renderProposalTemplate(template: string, data: ProposalData): string {
  ...
  rendered = rendered.replace(/#\[Requirement Hash\]/g, data.requirement ? `#${data.requirement}` : '#[Requirement Hash]');
  ...
  // Dependencies table
  const depRows = data.context.dependencies.map(dep =>
    `| #${dep.hash} | ${dep.type} | ${dep.description} |`
  ).join('\n');
  ...
  const filesRows = data.filesAffected.map(file =>
    `| \`${file.file}\` | ${file.action} | ${file.description} |`
  ).join('\n');
  rendered = rendered.replace(/\| `src\/\[path\]\/\[file\]\.ts` \| create\/modify \| \[Brief change description\] \|/g, filesRows);
