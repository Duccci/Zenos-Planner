...
describe('createEntityActionHandler (unit)', () => {
  ...
  it('returns not-implemented when registry is not provided', async () => {
    ...
    const textContent = res.content?.find((c: any) => c.type === 'text') as
  ...
  it('dispatches to action handler and validates output envelope', async () => {
    ...
    const actionHandlers = {
      alpha: async (payload: Record<string, unknown> | undefined) =>
        ({ success: true, data: { ok: true } }) as FunctionResult,
        ...
        ({
          ...
          error: {
            ...
            context: {
              ...
              issues: [{ path: 'flag', message: 'Expected boolean', code: 'invalid_type' }],
            },
          },
        }) as FunctionResult,
      beta: async () => ({ success: true, data: { ok: true } }) as FunctionResult,
