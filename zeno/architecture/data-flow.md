...
flowchart TD
    ...
    GenerateGates --> StoreGates[(Store Gates<br/>SQLite + Files)]
    
    StoreGates --> SelectGate{User Selects Gate}
    
    SelectGate --> DecomposeReqs[Decompose into Requirements<br/>Hierarchical Tree]
    
    DecomposeReqs --> StoreReqs[(Store Requirements<br/>SQLite with Hashes)]
    ...
    class StoreGates,StoreReqs,StoreArch,StoreRepos,StoreDepGraph,StoreProposals,StoreFailed,UpdateStatus storageStyle
    class DeepAnalysis,ZenoAlgo,GenerateGates,DecomposeReqs,GenArchitecture,DetectRepos,ScaffoldRepos,BuildDepGraph,GenProposals,AutoChecks,QualityCheck,GitCommit,GateRelease processStyle
...
### Phase 3: Gate Generation
1. Zeno's paradox algorithm generates gate sequence
2. Each gate closes half remaining distance to end state
3. Gates stored in SQLite (metadata) + Files (PRDs)

### Phase 4: Gate Execution Loop
For each gate:

#### 4a. Requirement Decomposition
1. User selects gate to work on
2. Gate decomposed into hierarchical requirements tree
3. Requirements stored in SQLite with content-addressable hashes
...
| Gate | Decomposition | Requirements Tree |
