...
describe('archive-execution coverage', () => {
  ...
  describe('calculateNextGateId', () => {
    ...
    it('falls back to 1 when gate id has no dash-separated number', () => {
      // Covers the `?? '1'` fallback branch on line 10
      const result = calculateNextGateId('noDash')
      expect(result).toBe('gate-02') // parseInt('1') = 1, next = 2
    })
  })
